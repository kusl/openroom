...
$current_time = new ClockTime(0, 0, 0);
$last_time = new ClockTime(23, 59, 59);
...
if ($_SESSION['username'] != '') {
    ...
    while ($group = mysql_fetch_array($groups)) {
        ...
        if ($group['roomgroupid'] == $_POST['group']) {
            $selected_str = 'class="selected"';
        }
                ...
                foreach ($reduced_reservations as $reservation) {
                    ...
                    if ((int) $reservation->roomid == (int) $room->id && ($res_collision == 'stalactite' || $res_collision == 'bat' || $res_collision == 'spelunker' || $res_collision == 'salamander' || $res_collision == 'stalagmite')) {
                        ...
                        if ($_SESSION['username'] != (string) $reservation->username && $isadministrator != 'TRUE') {
                            ...
                            if ($isadministrator == 'TRUE' || $_SESSION['username'] == (string) $reservation->username) {
                                $info .= '<strong>Time of Request</strong>: '.$reservation->timeofrequest."<br/><br/><center>Cancel this reservation? <a href=\'javascript:cancel(".$reservation->id.','.$_POST['group'].");\'>Yes</a> <a href=\'javascript:closePopUp();\'>No</a></center>";
                            }
