...
<body>
	...
	<br/><br/><strong>Daily Cancellation Report for the period starting: <?php echo $from; ?> and ending: <?php echo $to; ?></strong><br/><br/>
		...
		<?php
            ...
            while ($fromstamp <= $tostamp) {
                ...
                foreach ($roomname_array as $key => $roomid) {
                    $rescount = mysql_num_rows(mysql_query('SELECT reservationid, UNIX_TIMESTAMP(start), UNIX_TIMESTAMP(end), roomid, username, timeofrequest FROM cancelled WHERE (UNIX_TIMESTAMP(start) >= '.$fromstamp.' AND UNIX_TIMESTAMP(start) < '.strtotime('+1 day', $fromstamp).') AND roomid='.$roomid_array[$key].$orderbystr.';'));
                    echo '<td>'.$rescount.'</td>';
