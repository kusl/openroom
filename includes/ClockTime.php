...
//constructor
public function ClockTime($h, $m, $s)
{
    if ($h < 0) {
        $h * (-1);
    }
...
public function getTime()
{
    ...
    public function setMySQLTime($hhmmss)
    {
        if (is_string($hhmmss)) {
    ...
    public function addTime($hrs, $mins, $secs)
    {
    ...
    public function addMySQLTime($hhmmss)
    {
        $firstColon = strpos($hhmmss, ':');
        $secondColon = strpos($hhmmss, ':', $firstColon + 1);
    ...
    public function isGreaterThan($clockTime)
    {
    ...
    public function isEqualTo($clockTime)
    {
        if ($this->hours == $clockTime->hours) {
            if ($this->minutes == $clockTime->minutes) {
                if ($this->seconds == $clockTime->seconds) {
